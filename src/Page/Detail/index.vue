            ...
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  ...
                  :key="item1.id"
                  @click="changeActive(item1, item.spuSaleAttrValueList)"
...
export default {
  ...
  mounted() {
    this.getGoodsDetail(this.skuId);
  },
  computed: {
    skuId() {
  ...
  methods: {
    ...
    changeActive(item1, spuSaleAttrValueList) {
    ...
    inputSkuNum(event) {
      if (event.target.value >= 0) { 
        this.skuNum =Math.floor(event.target.value*1);
      } else {
        this.skuNum = 1;
      }
    },//输入商品数量
    async toCart(){
      try {
        ...
        sessionStorage.setItem('addShopcart',JSON.stringify(this.skuInfo))
